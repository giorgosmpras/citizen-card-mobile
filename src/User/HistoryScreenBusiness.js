...
// A SCREEN LISTING THE BUSINESSES THAT HAVE SCANNED THE USER
export default function HistoryScreenBusiness({ navigation }) {
  ...
  // In the first render get all the businesses from the user_record
  useEffect(() => {
    const getData = async () => {
      setLoading(true);
      ...
      // reverse them so that the last scan appears first
      setData(json.user_business_record.reverse());
      console.log(data);
    };
          ...
          renderItem={({ item }) => (
            <Card style={styles.renderContainer}>
              <View style={{ width: "100%" }}>
                <Text style={styles.title}>{item.business_title}</Text>
